...
const Index = () => {
  const [heroImage, setHeroImage] = useState(
    "/lovable-uploads/a773ac2f-9e06-49da-a3b9-b4425905b493.png"
  );
  const [isMaintenanceActive, setIsMaintenanceActive] = useState(false);
  const [maintenanceTarget, setMaintenanceTarget] = useState<string | null>(
    null
  );
  ...
  const loadMaintenanceStatus = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select(
          "is_enabled, maintenance_start, maintenance_end, countdown_message, title, message"
        )
        .single();
      ...
      setMaintenanceTarget(
        active && data.maintenance_end ? data.maintenance_end : null
      );
              ...
              {isMaintenanceActive && maintenanceTarget ? (
                <div className="flex flex-col w-full max-w-none px-0 sm:px-8 min-h-[80vh] sm:min-h-0 sm:max-w-4xl justify-center sm:justify-start">
                  ...
                  <div className="mt-1 sm:mt-10 flex flex-col items-center gap-2 sm:gap-2 pb-2 sm:pb-8 px-2 sm:px-0 w-full">
                    ...
                    <div className="w-full flex pb-12 pt-1 justify-center">
